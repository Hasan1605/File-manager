...
const Functionality = (Type, input, reader, title, content) => {
...
export default function Div_input(props) {
    ...
    return (<div className={props.mode ? styles.div_input_outer : styles.div_input_outer_Dark} onKeyPress={(event) => {
        if (event.key === 'Enter')
            Functionality(props.Type, input, reader, title, content)
    }}>
        <div className={styles.div_input_inner} >
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "center", marginTop: "30px" }} >
                ...
                <button onClick={() => { Functionality(props.Type, input, reader, title, content) }} >Create {props.Type}</button>
            </div>
        </div>
        <div />
    </div>
